...
shns() {
  ...
  if [ "${SHELLNS_CURRENTPROMPT_FUNCTION}" != "" ]; then
    shellNS_completion_function_tip_clear
    echo -ne "\e[2K"

    "${SHELLNS_CURRENTPROMPT_FUNCTION}" "${SHELLNS_CURRENTPROMPT_FUNCTION_ARGUMENTS[@]}"
    shellNS_output_show
  fi
}
#
...
_shns() {
  ...
  case "${execMode}" in
    ...
    "function")
      shellNS_completion_function_get_manual # <<--SEGUIR DAQUI
      if [ "${SHELLNS_CURRENTPROMPT_FUNCTIONS_MANUALS["${SHELLNS_CURRENTPROMPT_FUNCTION}"]}" != "" ]; then
        shellNS_completion_keybind_arrows_bind
        shellNS_completion_function_tip_clear
